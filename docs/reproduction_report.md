...
## 9. v21 变更记录（2026-08-31，作者本机收编）

> 本章记录作者在本机（macOS）代码库中对本报告 §6 缺陷清单的收编结果，以及收编过程中
> **新发现**的问题。至此发布版与复现验证版收敛为同一套代码。

### 9.1 复现报告 9 项缺陷的收编状态

| # | 缺陷 | 本机处理 | 状态 |
|---|---|---|---|
| 1 | `sci_rank_experiment.py` 硬编码 parquet 路径 | 本机已改为调用 `sci_g_series_fast.g_n_raw_series_fast`（复现前即完成）| 已修复 |
| 2 | `sci_rank_figs.py` 无 error 过滤 / 空桶除零 | 加载时过滤无 `hit`/`G` 的失败条目并打印告警；两处分桶加空桶保护；CV 分布加 `shape` 缺失容错 | 已修复 |
| 3 | `sci_gplots_50.py` verdict 缺失 → KeyError | `fav_stats.setdefault(verdict, [])` 降级 | 已修复 |
| 4 | `_sci_gseries_sorted.py` 缺 `__main__` guard | 该脚本为复现方自建桥接脚本，不属发布范围，本机不适用 | 不适用 |
| 5 | `lib/chartjs/chart.umd.min.js` 缺件 | 本机存有（205KB），已加入打包白名单 | 已修复 |
| 6 | `category_map.duckdb` / `binary_gmax` 缺件 | `binary_gmax_20260812.json`（1.6KB）已随包；`category_map.duckdb`（149MB）见 §9.3 | 部分（受平台限制）|
| 7 | 纯英文长句被判为单短语 → RBO 恒空 | `should_skip_llm` 增加「单段 >4 词强制走 LLM 编排」 | 已修复 |
| 8 | `_SKIP_RE` 缺括号 | 字符集补 `()`，兼容 `attention mechanism(s)` | 已修复 |
| 9 | `annotate.py` 单写 raw/normalized | `annotate_year` 改为双写：`--normalize` 时同时落 raw（RBO 源）与 normalized（可视化输入）| 已修复 |

**修复 7/8 的行为验证**（对比修复前后，`should_skip_llm`）：

| 输入 | 旧（bug）| 新（修复）| 说明 |
|---|---|---|---|
| `transformer attention in language models` | True | **False** | 长句交 LLM 编排，不再整体当短语送 RBO（原导致恒空）|
| `attention mechanism(s)` | False | **True** | 括号词形被正确识别为合法短语 |
| `attention mechanism, surface code` | True | True | 逗号分割短语，行为不变 |
| `quantum computing` | True | True | 单词短语，行为不变 |
| `black hole, neutron star, gravitational wave` | True | True | 三短语，行为不变 |
| `量子纠错表面码` | False | False | 中文走 LLM，行为不变 |

### 9.2 收编过程中新发现的问题（本报告未列）

1. **`ai_filter.py` 依赖缺失（打包缺陷）**：`sci_seeds_balanced.py` 会 `import ai_filter`，
   但旧打包白名单未包含它 —— 任何人拿到包运行该脚本都会 `ImportError`。
   该模块仅依赖标准库 `argparse`，已补入白名单。
2. **`.gitignore` 漏打包的根因**：`SHARE_MANIFEST.md` 早已声明包含 `.gitignore`，
   但 `make_share_package.py` 的 `build_stage()` 根文件列表里始终没有它 ——
   这是**文档与代码不一致**，而非单纯的"忘记上传"。已修正打包脚本本身。
3. **自检盲区**：原打包自检只校验「白名单精确性」（文件在不在），
   不校验「依赖闭合性」（import 能不能解析）。第 1 条缺陷正因此长期潜伏。
   已新增依赖闭合自检：静态解析全部 import，逐条比对包内模块 / 标准库 / requirements。
4. **隐私扫描的第二个盲区：按扩展名跳过「二进制」**。发布前的隐私扫描把 `.parquet`
   当作二进制文件跳过，但 parquet 内实际是论文文本。改用 `git ls-files | xargs grep -l`
   （不区分二进制）扫描暂存区后，1994/1995 两份 parquet 命中形如 `/home/<user>/...`、
   `/Users/<user>/...` 的路径。经核查均为**论文正文自带的目录/FTP/邮箱**（1990 年代论文常见），
   源自 Kaggle 官方数据集，与本机无关，判定为误报、不影响发布。
   **教训**：隐私扫描不得以扩展名为由跳过数据文件——对二进制也须 `grep -a`
   或读取文本列扫描，否则真泄漏时会漏检。

### 9.3 `category_map.duckdb` 不随包的技术原因

该文件 **149MB，超过 GitHub 单文件 100MB 硬限制**，无法通过 Git 分发。处理方式是补全可复现路径，
而非简单"不分享"：

- 随包提供生成器 `scripts/build_category_map.py`（配 `category_map_source_db`
  或环境变量 `SCI365_SOURCE_DB` 即可重建）；
- `web/explore.py` 对该库**已具备优雅降级**（`os.path.exists` 判空 → 关闭领域归一，不崩溃），
  因此缺失不影响主链路；
- `SHARE_MANIFEST.md` 已明确标注为「可选重建产物」，并写明超限原因。

### 9.4 v21 自检结果（打包后独立执行，不由打包脚本自证）

| 自检项 | 方法 | 结果 |
|---|---|---|
| 依赖闭合性 | 静态解析 245 条 import，比对包内 33 个模块 / 标准库 / requirements | 全部可解析 |
| 隐私泄漏 | 逐行扫描 98 个文本文件，匹配 `/Users/`、`/home/`、Windows 盘符路径、`sk-` key | 0 命中 |
| 语法编译 | `compileall` 全量 `scripts/` + `web/` | 通过 |
| 体积合规 | 单文件 >100MB 检查（GitHub 硬限制） | 无 |
| 打包内建校验 | 白名单精确性 + 缺件检查（`.gitignore`/`lib/chartjs`/`binary_gmax`）| 14 项全过 |
