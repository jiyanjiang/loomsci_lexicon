...
def main():
    ...
    # 每学科选 200/47 ≈ 4-5 个节点（AI 类限量）
    # AI 相关类只给少数
    ai_fields = {"cs-AI", "cs-CV", "cs-CL", "stat-ML"}
    seeds = []
    seen_ph = set()   # 全局去重
    for field, cats in FIELD_MAP.items():
        top_n = 2 if field in ai_fields else 4
        phs = field_phrases(cats, 2015, top_n * 2)
        # 跨学科去重 + 每学科取前 top_n
        picked = 0
        for ph, n in phs:
            if ph in seen_ph:
    ...
    json.dump(seeds, open("data/sci/discovery/randomwalk/seeds_balanced.json", "w"),
              ensure_ascii=False, indent=1)
    print(f"\n→ data/sci/discovery/randomwalk/seeds_balanced.json ({__import__('time').time()-t0:.0f}s)")
