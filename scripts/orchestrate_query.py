...
# 跳过判断：纯英文（可含数字/连字符/括号）+ 逗号分割
# 2026-08-31 v21（Windows 复现报告收编）：字符集补 () —— 兼容
# attention mechanism(s) 这类词形，否则带括号的合法短语被误判为「需 LLM」。
...
def should_skip_llm(query: str) -> bool:
    ...
    2026-08-31 v21（Windows 复现报告收编）：
      单段（无逗号）且 > 4 词 → 判为自然语言长句，**强制走 LLM 编排**。
      旧逻辑把整句当作 1 个短语（parts=1 满足 1<=1<=5）直接送进 RBO，
      而 RBO 检索的是学术短语，整句永远命中不了 → 结果恒空。
    """
    q = query.strip()
    if not q:
        return False
    ...
    if not (1 <= len(parts) <= MAX_PHRASES):
        return False
    # 单段长句（>4 词）视为自然语言，交 LLM 编排拆分为短语
