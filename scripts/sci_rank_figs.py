...
ok = json.load(open("data/sci/discovery/randomwalk/rank_2000.json"))
...
for i, (key, name, color) in enumerate(rules):
    ...
    for lo, hi in buckets:
        sub = ok_sorted[lo:hi]
        rates.append(sum(1 for r in sub if r["hit"]) / len(sub) * 100)
...
cv_hit = [r["shape"]["f_cv"] for r in hits]
cv_miss = [r["shape"]["f_cv"] for r in nonhits]
