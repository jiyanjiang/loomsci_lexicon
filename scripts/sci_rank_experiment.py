...
def worker(args):
    ...
    try:
        ...
        return {"A": A, "B": B, "I": float(I), "hit": hit,
...
def main():
    ...
    cands = json.load(open(f"{OUT}/cands_2000.json"))
    # 分批：batch 0 → 前 1000，batch 1 → 后 1000
    lo = batch * 1000
    hi = min(lo + 1000, len(cands))
    batch_cands = cands[lo:hi]
    print(f"批次 {batch}: 候选 {len(batch_cands)}（{lo}-{hi}）", flush=True)

    from concurrent.futures import ProcessPoolExecutor, as_completed
    t0 = time.time()
    jobs = [(c["A"], c["B"], c["I"]) for c in batch_cands]
    results = []
    ...
    out = f"{OUT}/rank_batch{batch}.json"
